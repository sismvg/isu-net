...
public:
	...
	virtual shared_memory invoke(const const_memory_block&) const = 0;
	virtual shared_memory operator()(const const_memory_block&) const = 0;
};
...
public:
	...
	{//没有模板lambda的临时解决方案
		...
		const shared_memory& results() const
		{
	...
	private:
		...
		shared_memory _results_memory;
		const_memory_block _memory;
	};

	virtual shared_memory invoke(const const_memory_block& memory) const
	{
		/*
			流程
			反序列化参数
	...
	virtual shared_memory operator()(const const_memory_block& memory) const
	{
