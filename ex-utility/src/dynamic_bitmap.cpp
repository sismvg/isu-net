...
{
	return have_bit(pos) && just_test(pos) == true;
}

bool dynamic_bitmap::have_bit(size_t pos) const
{
	return _set.size() > pos;
}
...
bool dynamic_bitmap::just_set(size_t pos, bool val)
{
	bool ret = _set.test(pos);
	_set.set(pos, val);
	return ret;
}

bool dynamic_bitmap::set(size_t pos, bool val)
{
	_try_extern(pos);
	return just_set(pos, val);
}
