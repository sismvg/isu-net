...
#include <array>
#include <sul-serialize/meta_archive.hpp>
#include <sul-serialize/archive_size.hpp>
#include <ex-utility/memory/shared_memory_block.hpp>

/*
	serialize_meta_process用于更加精细的模板变参的递归控制
*/
...
private:
	static const std::size_t size_of_argument = 20;
	size_t _index;
	size_t _myindex;
	std::array<size_t, 20> _sizes;
	std::array<serialize_object, 20> _objects;
...
public:
	...
	template<class... PassedArg>
	shared_memory archive(const PassedArg&... args)
	{
		return archive_real(
			type_list<PassedArg...>(), args...);
	}
	...
	template<class... RealArg, class... PassedArg>
	shared_memory archive_real(type_list<RealArg...> typelist,
		const PassedArg&... args)
	{
