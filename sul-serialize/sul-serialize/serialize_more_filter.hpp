...
#define CONDITION_CALL_FILTER_FALSE(name)\
	...
	SERALIZE_OBJECT_TEPMLATE
	static auto _call_impl_archive(T& value, std::true_type)
	->decltype(PreLayerFilter::archive_filter SERALIZE_TEMPLATE_USE(value))
	{
	return PreLayerFilter::archive_filter SERALIZE_TEMPLATE_USE(value);
	}

	SERALIZE_OBJECT_TEPMLATE
	static std::pair<bool,T*> _call_impl_archive(T& value, std::false_type)
	{
		return std::make_pair(false,nullptr);
	}

	SERALIZE_OBJECT_TEPMLATE
		static auto _call_impl_rarchive(T& value, std::true_type)
		->decltype(PreLayerFilter::rarchive_filter SERALIZE_TEMPLATE_USE(value))
	{
		return PreLayerFilter::rarchive_filter SERALIZE_TEMPLATE_USE(value);
	}

	SERALIZE_OBJECT_TEPMLATE
		static std::pair<bool, T*> _call_impl_rarchive(T& value, std::false_type)
	{
		return std::make_pair(false, nullptr);
	}
public:
	...
	:public meta_functional
{
	...
	struct functional
	{
		static const bool value = !is_no_need_copy<T>::value;
		typedef std::integral_constant<bool, value> type;
	};
