...
#include <isu-net/trans_core_def.hpp>
...
public:
...
#endif
	{
		...
		struct v
		{
			static DWORD WINAPI route(LPVOID ptr)
			{
				reinterpret_cast<trans_core_impl*>(ptr)->_recver_thread();
				return 0;
			}
		};
		_handle = CreateThread(0, 0,v::route, this, 0, NULL);
	}
	...
	*/
	bool cancel_io(const io_mission_handle& handle, bool active_io_failed = true)
	{
		try
		{
			if (!_received[handle->address].test_and_set(handle->id))
			{
				_exit_timer(*handle.get(), handle->timer, active_io_failed);
				handle->retry_limit = -1;
			}
		}
		catch (...)
		{
	...
	*/
	io_result assembile_send(const shared_memory& memory,
		const core_sockaddr& addr,const retrans_control* ctrl_ptr=nullptr)
	{//trunk只允许trunk+数字编号命名
		...
		*/
		...
		{
			retrans_trunk* trunk2 = new retrans_trunk(group, memory, addr);
			trunk2->retry_limit = ctrl.retry_limit;
			io_mission_handle io_handle(trunk2);

			timer_handle handle =
				_timeout_timer.ready_timer(new io_mission_handle(io_handle),
				ctrl.first_timeout);

			group.trunk.local_handle = handle;
			result = io_result(io_handle, 0, 0);
		}
		else
		{
			group.magic |= PACKAGE_FROM_MULTICAST_ADDR;
		}
	...
	io_result send_assembiled(const shared_memory& memory,const ipaddr& addr)
	{
		...
		if (!is_multicast(addr))
		{
			auto handle = head->trunk.local_handle;

			auto io_handle = *reinterpret_cast<io_mission_handle*>(
				_timeout_timer.get_argument(handle));
			_timeout_timer.start_timer(head->trunk.local_handle);

			_sendto_impl(native(), memory, 0, addr);
			return io_result(io_handle, 0, WSAGetLastError());
		}

		_sendto_impl(native(), memory, 0, addr);
		return io_result(nullptr, 0, WSAGetLastError());
	}

	//发送方必须预留trans_core_reserve个字节的头部
	io_result send(const shared_memory& memory,
		const core_sockaddr& addr,const retrans_control* ptr)
	{
...
private:
	...
	std::map<ipaddr, dynamic_bitmap> _received;
	...
	void _stun_group(retrans_trunk* trunk,io_mission_handle handle)
	{
		if (_bad_send)
	...
	void _exit_timer(const retrans_trunk& trunk,
	...
	{
		auto* io_handle = reinterpret_cast<io_mission_handle*>(ptr);
		auto* trunk = io_handle->get();
#ifdef _USER_DEBUG_LOG
		mylog.log(log_debug, "retry");
#endif
		if (++trunk->retry_count > trunk->retry_limit)
		{
			this->_stun_group(trunk, *io_handle);
	...
	{
		auto* trunk =
			reinterpret_cast<io_mission_handle*>(ptr);
		delete trunk;
	}

	_trans_group _make_group(package_id_type id, const trans_trunk& trunk)
	{
		_trans_group result;  
		...
		while (true)
		{
			...
			if (head.magic&SLICE_WITH_ACKS)
			{
				...
				if (_packages.have(head.id) )
				{
					...
					if (_work_flag&POST_SEND_COMPLETE)
					{
						auto* ptr = reinterpret_cast<io_mission_handle*>(arg);
						_post(io_result(*ptr),
			...
			{		
				...
				if (!(head.magic&PACKAGE_FROM_MULTICAST_ADDR))
				{
					_send_ack(from, head);
				}
				if (!_received[from].test_and_set(head.id))
				{
					_post(io_result(nullptr, 0, error_code),
	...
	shared_memory _get_shared_memory(shared_memory& memory, size_t data_size)
	{
		...
		if (data_size < size / 2)
		{
			...
			mymemcpy(buffer, memory.get(), data_size);
			size = data_size;
...
trans_core::trans_core(const core_sockaddr& addr,size_t limit_thread)
{
	_impl.reset(new trans_core_impl(addr,
		trans_core_impl::bad_send_functional(), limit_thread));
}

trans_core::trans_core(const core_sockaddr& addr,
...
io_result trans_core::assemble_send(const shared_memory& memory,
	const core_sockaddr& dest, const retrans_control* ctrl_ptr)
{
	return _impl->assembile_send(memory, dest, ctrl_ptr);
}

io_result trans_core::send_assembled(const shared_memory& memory, const core_sockaddr& dest)
{
