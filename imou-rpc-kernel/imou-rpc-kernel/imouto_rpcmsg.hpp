...
struct rpc_call_body
{
	...
	//WAR.指针是否复制值还不明确
	XDR_DEFINE_TYPE_4(rpc_call_body,
		rpcstub_ident, body, void*, trunk,
		verify_ident, ident,void*,handle)
};
...
//所有的消息组按照都必须能正确的被这个函数遍历
template<class MemBlk,class Func>
void for_each_memory_block(MemBlk block, Func fn)
{
	while (block.size)
	{
		std::size_t size = 0;
		advance_in(block, rarchive(block, size));

		fn(MemBlk(block.buffer, size));

		advance_in(block, size);
	}
}
